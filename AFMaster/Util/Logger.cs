...
namespace AFMaster.Util
{
    public static class SimpleLog
    {
        ...
        ///     <see cref="Severity.Info" />, <see cref="Severity.Warning" />, <see cref="Severity.Error" /> and
        ///     <see cref="Severity.Exception" />
        ///     are actually written to the log file. When log level is set to e.g. <see cref="Severity.Error" />, only
        ///     entries with severity <see cref="Severity.Error" /> and <see cref="Severity.Exception" /> are actually written to
        ///     the log file.
        ...
        ///     Log file is assembled in <see cref="GetFileName" /> using
        ///     <code>string.Format("{0}\\{1}{2}{3}.{4}", LogDir, Prefix, dateTime.ToString(DateFormat), Suffix, Extension)</code>.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        ///     Default is "yyyy_MM_dd" (e.g. 2013_04_21), which leads to a daily change of the log file. Set to null to return to
        ///     default. Set to e.g. "yyyy_MM_dd_HH" to change log file hourly.
        ///     Log file is assembled in <see cref="GetFileName" /> using
        ///     <code>string.Format("{0}\\{1}{2}{3}.{4}", LogDir, Prefix, dateTime.ToString(DateFormat), Suffix, Extension)</code>.
        /// </remarks>
        public static string DateFormat
        {
            get => _dateFormat ?? "yyyy_MM_dd";
        ...
        /// </summary>
        /// <remarks>
        ///     Log all entries with <see cref="Severity" /> set here and above. In other words, do not write entries to the log
        ///     file with
        ...
        /// <summary>
        ///     When <see cref="WriteText" /> is true, this is the separator text entries reperesenting attributes or values are
        ///     separated with. Defaults to " | ".
        /// </summary>
        public static string TextSeparator
        {
            get => _textSeparator;
            set => _textSeparator = value ?? string.Empty;
        }
        ...
        /// </summary>
        /// <remarks>
        ///     Is assembled from <see cref="LogDir" />, <see cref="Prefix" />, the current date and time formatted in
        ///     <see cref="DateFormat" />,
        ///     <see cref="Suffix" />, "." and <see cref="Extension" />. So, by default, the file is named e.g. "2013_04_21.log"
        ...
        ///     When <see cref="logDir" /> is set and it cannot be created or writing a first entry fails, no exception is thrown,
        ...
        ///     Here, <see cref="LogDir" /> is created, when it does not exist.
        /// </param>
        ...
        ///     <see cref="Extension" /> for details. When null is passed here, <see cref="Extension" /> is not
        ///     set.
        /// </param>
        /// <param name="dateFormat">
        ///     <see cref="DateFormat" /> for details. When null is passed here, <see cref="DateFormat" /> is
        ///     not set.
        /// </param>
        /// <param name="logLevel">
        ///     <see cref="LogLevel" /> for details. When null is passed here, <see cref="LogLevel" /> is not
        ///     set.
        /// </param>
        ...
        /// </param>
        /// <param name="check">
        ///     Whether to call <see cref="Check" />, i.e. whether to write a test entry after setting the new log
        ///     file. If true, the result of <see cref="Check" /> is returned.
        /// </param>
        /// <param name="writeText">
        ///     <see cref="WriteText" /> for details. When null is passed here, <see cref="WriteText" /> is not
        ///     set.
        /// </param>
        /// <param name="textSeparator">
        ///     <see cref="TextSeparator" /> for details. When null is passed here,
        ///     <see cref="TextSeparator" /> is not set.
        /// </param>
        /// <returns>Null on success, otherwise an exception with what went wrong.</returns>
        public static Exception SetLogFile(string logDir = null, string prefix = null, string suffix = null,
            string extension = null, string dateFormat = null, Severity? logLevel = null, bool? startExplicitly = null,
            bool check = true, bool? writeText = null, string textSeparator = null)
        {
        ...
        /// </summary>
        /// <param name="logDir">
        ///     The logging diretory to set. When passing null or the empty string, the current working directory
        ///     is used.
        /// </param>
        ...
        ///     Whether to use the background task (thread) to write messages to disk. Default is true.
        ///     This is much faster than writing directly to disk in the main thread.
        /// </param>
        /// <returns>
        ///     Null on success or the <see cref="Exception" /> that occurred when processing the message, i.e. when enqueuing
        ///     the message (when <paramref name="useBackgroundTask" /> is true) or when writing the message to disk (when
        ...
        ///     How many frames to skip when detecting the calling method, <see cref="GetCaller" />. This is
        ///     useful when log calls to <see cref="SimpleLog" /> are wrapped in an application. Default is 0.
        /// </param>
        /// <returns>
        ///     Null on success or the <see cref="Exception" /> that occurred when processing the message, i.e. when enqueuing
        ///     the message (when <paramref name="useBackgroundTask" /> is true) or when writing the message to disk (when
        ...
        public static Exception Log(Exception ex, bool useBackgroundTask = true, int framesToSkip = 0)
        {
            return ex == null
                ? null
                : Log(GetExceptionXElement(ex), Severity.Exception, useBackgroundTask, framesToSkip);
        }
        ...
        ///     Tags for <see cref="Exception.Data" /> are added. Specific properties of the exception types
        ...
        public static XElement GetExceptionXElement(Exception ex)
        {
            ...
            xElement.Add(new XAttribute("Source",
            ...
            if (ex.Data.Count > 0)
            {
                ...
                foreach (DictionaryEntry de in ex.Data)
                    xDataElement.Add(new XElement("Entry", new XAttribute("Key", de.Key),
            ...
            xElement.Add(ex.InnerException == null
                ? new XElement("StackTrace", ex.StackTrace)
                : GetExceptionXElement(ex.InnerException));
        ...
        ///     <paramref name="useBackgroundTask" /> is false).
        /// </returns>
        public static Exception Log(string message, Severity severity = Severity.Info, bool useBackgroundTask = true,
            int framesToSkip = 0)
        {
            return string.IsNullOrEmpty(message)
                ? null
                : Log(new XElement("Message", message), severity, useBackgroundTask, framesToSkip);
        }
        ...
        ///     However, writing to the file is synchronized between threads. I.e. writing directly can be done from multiple
        ///     threads.
        ...
        ///     <paramref name="useBackgroundTask" /> is false).
        /// </returns>
        public static Exception Log(XElement xElement, Severity severity = Severity.Info, bool useBackgroundTask = true,
            int framesToSkip = 0)
        {
        ...
        {
            return string.Format("{0}\\{1}{2}{3}.{4}", LogDir, Prefix, dateTime.ToString(DateFormat), Suffix,
                Extension);
        }
        ...
        /// </summary>
        /// <remarks>
        ///     Opens the default program to show text or XML files and displays the requested file, if it exists. Does nothing
        ///     otherwise.
        ///     When <see cref="WriteText" /> is false, a temporary XML file is created and saved in the users's temporary path
        ...
        public static void ShowLogFile(DateTime dateTime)
        {
            ...
            {
                fileName = string.Format("{0}Log_{1}.xml", Path.GetTempPath(),
        ...
        /// <summary>
        ///     Clear background task's log entry queue. I.e. remove all log messages waiting to be written to
        ///     <see cref="FileName" /> by the background task.
        /// </summary>
        ...
        private static Exception WriteLogEntryToFile(XElement xmlEntry)
        {
            ...
            try
            {
                return
                    new Exception(
                        string.Format(
                            "Could not write to file '{0}', because it was blocked by another thread for more than {1} seconds.",
                            FileName, secondsToWaitForFile));
            }
            catch (Exception ex)
            {
                return ex;
        ...
        /// 
        /// ]]>
        ///     This is a basic implementation so far. Feel free to implement your own if you need something more sophisticated,
        ///     e.g.
        ...
        /// </summary>
        /// <remarks>
        ///     The method is walking up the frames in the stack trace until the first method outside <see cref="SimpleLog" /> is
        ///     reached.
        ///     When log calls to <see cref="SimpleLog" /> are wrapped in an application, this may still not be the method where
        ///     logging
        ///     was called initially (e.g. when an exception occurred and has been logged). In that case set
        ///     <paramref name="framesToSkip" />
        ///     accordingly to get outside the wrapper method(s).
        /// </remarks>
        /// <param name="framesToSkip">
        ///     How many frames to skip when detecting the calling method. This is useful when log calls to
        ///     <see cref="SimpleLog" /> are wrapped in an application. Default is 0.
        /// </param>
